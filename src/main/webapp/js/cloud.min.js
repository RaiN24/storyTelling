...
function parseText(t) {
	tags = {};
	$.post("text.php",{text:t},function(result){
	    t=result;
	    count=t.length;
	    var e = {};
...
function generate() {
	...
	words = [], layout.stop().words(tags.slice(0, max = Math.min(tags.length, +d3.select("#max").property("value")))).start()
}
...
function downloadSVG() {
	...
	function e() {
		function t(t, n, a) {
			...
			function tooFar(n){
				if(bef==null){
					if(Math.abs(n.x-300)<step&&Math.abs(n.y-200)<step)
						return false;
					return true;					
				}
				// var befX=bef.x+e[0]>>1;
				// var befY=bef.y+e[1]>>1;
				if(Math.abs(n.x-300)<step&&Math.abs(n.y-200)<step){
					return false;
				}
			...
			for (var r, o, s, l = ([{
				...
				if (n.x = l + o, n.y = i + s, !(tooFar(n)||n.x + n.x0 < 0 || n.y + n.y0 < 0 || n.x + n.x1 > e[0] || n.y + n.y1 > e[1]) && (!a || !u(n, t, e[0])) && (!a || c(n, a))) {
		...
		return k.start = function() {
			function n() {
				...
				l(n, d, u), 
				t(a, n, r) && (c.push(n), b.word(n), r ? i(r, n) : r = [{
					x: n.x + n.x0,
					y: n.y + n.y0
				}, {
					x: n.x + n.x1,
